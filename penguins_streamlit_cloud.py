...
if penguin_file is None: # если файл не загружен пользователем, то применяем обученную модель на наших данных
    rf_pickle = open('random_forest_penguin.picle', 'rb')
    map_picle = open('output_penguin.picle', 'rb')
    rfc = pickle.load(rf_pickle)
    unique_penguin_mapping = pickle.load(map_picle)
    rf_pickle.close()
    map_picle.close()
    penguin_df = pd.read_csv('penguins.csv')

else:                   # ну а если загрузил то,обучаем заново на его файле
...
# после того как обучили модель польз файла или своего мы предлагаем пользователю внести свои параметры для предсказания
with st.form("пользовательские вводы"):
    island = st.selectbox("Остров проживания пингвинов", options=['Biscoe', 'Dream', 'Torgersen'])
    sex = st.selectbox("Пол", options=['Мужская особь', 'Женская особь'])
    bill_length = st.number_input('Длина клюва (мм)', min_value=0)
    bill_depth = st.number_input('Высота клюва (мм)', min_value=0)
    flipper_length = st.number_input('Длинна ласт (мм)', min_value=0)
...
new_prediction = rfc.predict([[bill_length, bill_depth, flipper_length,
                               body_mass, island_biscoe, island_dream, island_torgersen,
                               sex_female, sex_male]])
...
# запускаем сохраненный файл важности признаков

st.image('features_importance.png')

# гистограммы
st.write("Ниже приведены гистограммы распределений трех переменных "
